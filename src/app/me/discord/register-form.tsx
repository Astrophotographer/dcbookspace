...
function ChannelSwitch({
  ...
  if (phoneDigits.length >= 3) query.set("phone", phoneDigits);
  const suffix = query.toString() ? `?${query.toString()}` : "";

  return (
    <div className="mb-3 inline-flex rounded-full bg-[#ebe7dd] p-1">
      <Link
        href={`/me/telegram${suffix}`}
        aria-current={active === "telegram" ? "page" : undefined}
        className={cn(
      ...
      <Link
        href={`/me/discord${suffix}`}
        aria-current={active === "discord" ? "page" : undefined}
        className={cn(
          "inline-flex min-h-10 items-center rounded-full px-4 text-sm font-black transition",
          active === "discord"
            ? "bg-white text-blue-700 shadow-[0_4px_14px_rgba(31,39,38,0.08)]"
            : "text-[#65706d] hover:bg-white/50",
        )}
      >
        디스코드
