...
export function EditReservationGate(props: Props) {
    ...
    return (
      <div className="rounded-2xl border-2 border-red-200 bg-red-50 p-6 text-red-800">
        ...
        <p className="mb-4 text-sm">
          본인이 작성한 신청서만 수정할 수 있습니다. 신청서를 인쇄하거나
          상태만 확인하려면 인쇄 페이지로 이동해 주세요.
        </p>
        <div className="flex gap-2">
          <Link href={`/reservations/${props.reservationId}/print`}>
  ...
  if (!props.editable) {
    return (
      <div className="rounded-2xl border-2 border-amber-300 bg-amber-50 p-6 text-amber-900">
        ...
        <p className="mb-4 text-sm">
          이미 결재가 진행됐거나 마감된 신청서는 수정이 불가합니다. 변경이
          필요하면 신청서를 삭제 후 새로 작성해 주세요.
        </p>
        <div className="flex gap-2">
          <Link href={`/reservations/${props.reservationId}/print`}>
            <Button variant="secondary">인쇄 페이지로</Button>
